"""
Opportunity-Term Matcher (Local PDF Version)

Extracts deal terms from local PDFs and matches them against natural language
investment opportunity descriptions to find location and match reasoning.

Uses OpenAI to analyze PDF content directly instead of Google URL Context.
"""
...
class OpportunityMatch:
    ...
    opportunity_id: int
    location: str
    location_granularity: str  # 'city', 'area', 'building', 'region', 'corridor'
    match_reasoning: str
...
class OpportunityTermMatcher:
    ...
    def __init__(self, api_key: Optional[str] = None):
        """Initialize the matcher with OpenAI client."""
    ...
    def query_with_context(self, query: str, context: str) -> str:
        """Query OpenAI with document context."""
        try:
            # Truncate context if too long (max ~15k tokens = ~60k chars for GPT-4)
            max_context_chars = 50000
            if len(context) > max_context_chars:
                logger.warning(f"Context truncated from {len(context)} to {max_context_chars} chars")
                context = context[:max_context_chars] + "\n\n[Content truncated...]"

            messages = [
                {"role": "system", "content": "You are an expert at analyzing business documents and extracting key deal terms."},
    ...
    def extract_council_terms(self, council_doc: PDFDocument) -> Dict[str, Any]:
        ...
        # Extract PDF text
        pdf_text = self.extract_pdf_text(council_doc.file_path)
        if not pdf_text:
        ...
        try:
            response = self.query_with_context(query, pdf_text)

            return {
                'raw_terms': response,
                'document_name': council_doc.name,
                'document_path': council_doc.file_path
            }
        except Exception as e:
    ...
    def extract_investor_terms(self, investor_doc: PDFDocument) -> Dict[str, Any]:
        ...
        pdf_text = self.extract_pdf_text(investor_doc.file_path)
        if not pdf_text:
            return {'error': 'Could not extract PDF text'}
        ...
        try:
            ...
            return {
                ...
                'document_path': investor_doc.file_path
            }
        except Exception as e:
    ...
    def extract_provider_terms(self, provider_doc: PDFDocument) -> Dict[str, Any]:
        ...
        pdf_text = self.extract_pdf_text(provider_doc.file_path)
        if not pdf_text:
            return {'error': 'Could not extract PDF text'}
        ...
        try:
            ...
            return {
                ...
                'document_path': provider_doc.file_path
            }
        except Exception as e:
    ...
    def match_opportunity(
        ...
        try:
            messages = [
                {"role": "system", "content": "You are an expert at matching investment opportunities with deal terms from councils, investors, and service providers."},
                {"role": "user", "content": match_query}
            ]
            ...
            # Parse the response
            location, granularity, reasoning, details, confidence = self._parse_match_response(response_text)
...
if __name__ == "__main__":
    ...
    # Example documents using local file paths
