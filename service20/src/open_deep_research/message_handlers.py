...
class ResearchResultHandler(MessageHandler):
    ...
    async def handle_research_complete(
        ...
        from open_deep_research.database_storage import (
            ...
            create_service20_alert
        )

        # Store research results in database
        opportunity_data = payload.get('opportunity_data')
        record_id = await save_research_to_database(payload, opportunity_data)

        if record_id:
            logger.info(f"Saved research to database with ID: {record_id}")

            # Create alert notification in database
            alert_id = await create_service20_alert(payload, opportunity_data)
            if alert_id:
                logger.info(f"Created database alert with ID: {alert_id}")
            else:
                logger.warning("Failed to create database alert")

            # Create alert via CNZ API
            try:
                import sys
                from pathlib import Path
                # Add parent directory to path to import create_alert_api
                ...
                geoname_id = opportunity_data.get('geoname_id') if opportunity_data else None
                city_country_code = opportunity_data.get('city_country_code') if opportunity_data else None
                location = opportunity_data.get('location', 'Unknown') if opportunity_data else 'Unknown'
                title = opportunity_data.get('title', research_id) if opportunity_data else research_id

                api_result = create_service20_research_alert(
                ...
                if api_result:
                    logger.info(f"Created API alert successfully")
                else:
                    logger.warning("Failed to create API alert")

            except Exception as e:
                logger.error(f"Error creating API alert: {e}")
        else:
            logger.warning("Failed to save research to database")
